...
public class RabbitConfig {
    ...
    @Bean
    Queue queue() {
        return new Queue(DEVICE_SERVICE_QUEUE, true);
    }

    @Bean
    Binding bindRoomDeletedEvent(TopicExchange exchange, Queue queue) {
        return BindingBuilder.bind(queue).to(exchange).with(ROOM_DELETED_ROUTING_KEY);
    }
